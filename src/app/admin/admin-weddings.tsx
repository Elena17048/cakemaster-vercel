...
import { getWeddingPageContent, updateWeddingPageContent, uploadWeddingImage, deleteWeddingImage } from '@/lib/api';
import type { WeddingPageContent, WeddingReview } from '@/lib/types';
import { Loader2, PlusCircle, Trash2, Edit, UploadCloud } from 'lucide-react';

const ReviewForm = ({ review, onSave, onCancel, isPending }: { review?: WeddingReview | null, onSave: (reviewData: Omit<WeddingReview, 'id'>) => void, onCancel: () => void, isPending: boolean }) => {
...
export function AdminWeddings() {
    ...
    const [editingReview, setEditingReview] = useState<WeddingReview | null>(null);
    ...
    const { data: content, isLoading, isError } = useQuery<WeddingPageContent>({
        queryKey: ['weddingPageContent'],
        queryFn: getWeddingPageContent,
    });

    const updateMutation = useMutation({
        mutationFn: updateWeddingPageContent,
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['weddingPageContent'] });
            toast({ title: 'Success', description: 'Wedding page content updated.' });
        },
        onError: (error) => {
    ...
    const handleSaveReview = (reviewData: Omit<WeddingReview, 'id'>) => {
        if (!content) return;
        let updatedReviews: WeddingReview[];

        if (editingReview) {
            updatedReviews = (content.reviews || []).map(r => r.id === editingReview.id ? { ...r, ...reviewData } : r);
        } else {
            const newReview: WeddingReview = { ...reviewData, id: Date.now().toString() };
    ...
    const openEditReviewDialog = (review: WeddingReview) => {
    ...
    const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>, index: number) => {
        if (e.target.files && e.target.files[0] && content) {
            ...
            try {
                ...
                if (oldImageUrl) {
                    await deleteWeddingImage(oldImageUrl);
                }
                const newImageUrl = await uploadWeddingImage(e.target.files[0]);
                const updatedImages = [...(content.galleryImages || [])];
                updatedImages[index] = newImageUrl;
                
                // Fill any empty slots up to the current index to avoid sparse arrays
    ...
    const handleImageRemove = async (index: number) => {
        ...
        try {
            await deleteWeddingImage(imageUrlToRemove);
            const updatedImages = [...(content.galleryImages || [])];
            updatedImages[index] = ""; // Set to empty string instead of removing
            await updateMutation.mutateAsync({ ...content, galleryImages: updatedImages });
    ...
    if (isError) return <p className="text-destructive">Error loading wedding page content.</p>;
        ...
        <div className="grid gap-8 md:grid-cols-2">
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>Add, edit, or delete customer testimonials for the wedding page.</CardDescription>
                </CardHeader>
            ...
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>Update the three photos displayed on the wedding page.</CardDescription>
                </CardHeader>
                       ...
                       return (
                            <div key={index} className="space-y-2">
                                ...
                                <div className="flex items-center gap-4 p-2 border rounded-md">
                                    {imageUrl ? (
                                        <Image src={imageUrl} alt={`Wedding gallery image ${index + 1}`} width={64} height={64} className="rounded-md object-cover h-16 w-16" />
                                    ) : (
                                    ...
                                    <div className="flex-grow space-y-2">
                                        <label htmlFor={`upload-${index}`} className="w-full">
                                            ...
                                            <Input id={`upload-${index}`} type="file" className="sr-only" accept="image/*" onChange={(e) => handleImageUpload(e, index)} disabled={uploadingIndex !== null} />
                                        </label>
                                        {imageUrl && (
